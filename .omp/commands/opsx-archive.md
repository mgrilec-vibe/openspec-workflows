...
4. **Publish before archiving.** This is mandatory and failure leaves the change active.
   ...
   - Before moving `changeRoot`, use `git worktree list --porcelain` plus the absolute common Git directory (`git rev-parse --path-format=absolute --git-common-dir`) to identify the unique worktree whose absolute `git-dir` equals that common directory: the primary repository, never a detached release worktree. Require that it is checked out on the default branch and clean, run `git -C "<primary-worktree>" pull --ff-only origin "<default-branch>"`, then require its `HEAD` to equal `origin/<default-branch>` and to contain `release_commit`. Missing, ambiguous, wrong-branch, dirty, or non-fast-forward worktrees stop the archive without switching branches, stashing, resetting, or overwriting local work.
...
6. Summarize the archive path, spec-sync outcome, merged PR URL, closed issue URL, old/new version, release tag/URL, release commit, and synchronized primary-worktree path/HEAD. Include any confirmed artifact/task warnings.
